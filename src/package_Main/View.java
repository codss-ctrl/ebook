...
////////////////////////////////////////////////////////////////	
	...
	private void modifyPassword(){
		...
		if(iServiceImpl.userUpdate(userInfo) == 1){
			System.out.println("수정을 하였습니다.");
			return;
		}else{
			System.out.println("수정을 하지 못했습니다.");
		}
	}
	...
	private void rentListView(){
		...
		while(true){
			...
			if(userRentDetail.size() == 1){
				System.out.println("[1] 상세 보기");
			}else{
				System.out.println("[1 ~ " + userRentDetail.size() + "] 상세 보기");
			}
			System.out.println("[0] 뒤로 가기");
			System.out.println("===================================================\n");
			...
			if (input == 0) {
				return;
			}else if(0<input && input < userRentDetail.size()+1){
				rentBookDetail(userRentDetail.get(input-1));
			} else {
				System.out.println("잘못된 입력입니다. 다시 입력해 주세요.");
			}
		}
	...
	//@param iInput
	//return
	private void rentBookDetail(RentVO selRent){
		int seq = 0;
		List<RentVO> userRentDetail = iServiceImpl.rentListView(user.getUser_id());
		while(true){
			System.out.println("===================================================\n");
			
			BookVO selbook = iServiceImpl.bookSelector(selRent.getBook_seq());
			System.out.println("\t 책 이름 \t\t 작가 \t 내 평점 \n");
			System.out.println("---------------------------------------------------\n");
			System.out.print("\t"+selbook.getBook_name()+"\t");
			System.out.print(selbook.getAuthor()+"\t");
			System.out.println(selRent.getRent_grade());
			seq =selRent.getRent_seq();
	...
	private void giveGrade(RentVO selRent){
		int grade = 0;
		while(true){
			System.out.println();
			...
			if(grade<0 || grade > 10){
				...
				Map<String, Object> user_grade = new HashMap<>();
				BookVO selBook = iServiceImpl.bookSelector(selRent.getBook_seq());
				user_grade.put("book", selBook);
				user_grade.put("rent", selRent);
				user_grade.put("grade", grade);
				iServiceImpl.giveGrade(user_grade);
				return;
			}
		}
	}
	...
	//@param iInput
	//return int
	private void modifyGrade(RentVO selRent){
		int grade;
		while(true){
			System.out.println();
			System.out.println("수정하실 평점을 입력해주세요(1 ~ 10)");
			grade = iInput();
			if(0 < grade && grade < 11){
				selRent.setRent_grade(grade);
				return;
			}else{
				System.out.println("잘못된 입력입니다. 다시 입력해 주세요.");
			}
	...
	private void chargePoint(){
		while(true){
			...
			if (point > 0) {
				if(iServiceImpl.chargePoint(userInfo) == 1){
					return;
				}
				System.out.println("충전을 하지 못했습니다. 다시 충전해 주세요");
			}
		}
	}
	...
	private void notifyView(){
		while(true){
			...
			System.out.println("[2] 신간 도서");
			System.out.println("[3] 인기 도서");
			System.out.println("[0] 뒤로 가기");
	...
	private void userNotifyview(){
		...
		while(true){
			...
			if(notifyList.size()>0){
				...
				for(NotifyVO notify : notifyList){
					System.out.print(String.format("[%2d ] \t%-30s\t\t%6d   %-10s\n", 
															notify.getNotify_seq(), 
															notify.getNotify_title(), 
															notify.getReadCount(), 
															notify.getNotify_date()));
				}
					System.out.println();
			}else{
			...
			System.out.println("==============================================================");
			System.out.println("[1 ~ " + notifyList.size() + "]");
	...
	private void popularBookView(){
		List<BookVO> bookList = iServiceImpl.popularBookView();
		while(true){
			System.out.println("===========================================================");
			System.out.println("\t\t\t인기 도서 보기");
			System.out.println("===========================================================");
			System.out.println("  번호\t\t제목\t\t     작가        \t평점");
			System.out.println("-----------------------------------------------------------");

			for(int i=0; i<bookList.size(); i++){
				System.out.println(String.format("[%2d ]  %-20s\t%-15s\t %.1f  ", i+1, 
						bookList.get(i).getBook_name(), bookList.get(i).getAuthor(), bookList.get(i).getBook_grade()));
			}
			System.out.println("-----------------------------------------------------------");
			System.out.println("[ 0 ] 뒤로가기");
			System.out.println("===========================================================");
...
/////////////////////////////////////////////////////////////////////	
	...
	private void voucherView(){
		...
		while(true){
			...
			if(input == 0){
				...
				buyVoucher();
			}else if(1 < input && input < userInfoList.size()+2){
				userVoucherDetail(userInfoList.get(input-2));
			}else{
				System.out.println("잘못된 입력입니다. 다시 입력해 주세요.");
			}
		}
	...
	private void buyVoucher(){
		while(true){
			...
			System.out.println("-------------------------");
			List<VoucherVO> voucherList = iServiceImpl.readValidVoucher();
			for(int i=0; i< voucherList.size(); i++){
				System.out.print("[" + (i+1) + "] ");
				System.out.println(String.format("%7s\t%8s", voucherList.get(i).getV_name(),
			...
			if(input == 0){
				return;
			}else if(0<input && input < voucherList.size()+1){
				voucherInfo.put("user", user);
				voucherInfo.put("selVoucher", voucherList.get(input-1));
				voucherInfo.put("v_price", voucherList.get(input-1).getV_price());
				if(iServiceImpl.buyVoucher(voucherInfo)){
					System.out.println("선택하신 이용권을 구매하였습니다.");
				}else{
					System.out.println("선택하신 이용권을 구매하지 못했습니다");
				}
			}else{
				System.out.println("잘못된 입력입니다. 다시 입력해 주세요.");
			}
	...
	private void searchBookName(){
		...
		while(true){
			...
			if(bookList.size() == 0){
				System.out.println("입력하신 책 이름과 일치하는 책이 없습니다");
				return;
			}
	...
	void bookListView(List<BookVO> bookList){
		...
		if(user == null){
			...
			if(bookList.size() == 1){
				...
				System.out.println("[ 2 ~ " + (bookList.size()+1) + " ] 상세보기 선택");
			}
...
/////////////////////////////////////////////////////////////////////////////	
	...
	//@return 선택한 책 객체
		while(true){
			...
			System.out.println("평점 : " + selBook.getBook_grade());
	...
	*/
	...
	//@return 선택한 공지 객체 가져오기 
		while(true){
			...
			System.out.print("조회수:" + notify.getReadCount() + "\t");
	...
	//@return 추가 성공시 true, 실패 시 false반환
		...
		while(true){
			...
			int voucherPeriod = iInputAd();
			
			voucher.setV_period(voucherPeriod);
			System.out.println("가격 : ");
			int voucherPrice = iInputAd();
...
//			voucher = iServiceImpl.voucherSelector(selVoucher);
	...
	//@return 
		...
		while(true){
			...
			System.out.println( month + "월 상세 매출");
			System.out.println("=============================================================");
			System.out.println(" No       사용자                       이용권                    구매일                가격");
			System.out.println("-------------------------------------------------------------");
